...
[Project Repository](https://github.com/IIfaitdoux/ADS-Project-I-Efficiency-of-find-operations-based-on-BST-AVL-Splay)
...
## How to run
1. Open the terminal under the `code` folder
2. Run `make all` to build the program
3. Run `make run` to execute the program
4. Run `python table.py` to generate the runtime table
5. Run `python plot.py` to generate the runtime plots
6. Run `make clean` to clean the build files
